...
public class MyConfigManager
{
  ...
  public MyConfig getNewConfig(String fileName, String[] header)
  {
    ...
    MyConfig config = new MyConfig(getConfigContent(fileName), file, getCommentsNum(file), plugin);
    return config;
  }
  ...
  private File getConfigFile(String file)
  {
    ...
    if (file.contains("/")) {
      File configFile;
      if (file.startsWith("/"))
        configFile = new File(plugin.getDataFolder() + file.replace("/", File.separator)); else
  ...
  public void setHeader(File file, String[] header)
  {
    ...
    try
    {
      ...
      String currentLine;
      while ((currentLine = reader.readLine()) != null) { String currentLine;
    ...
    for (String line : lines)
    {
      if (line.startsWith(getPluginName() + "_COMMENT"))
      {
        ...
        if (comment.startsWith("# +-"))
        {
          ...
          String normalComment;
          if (comment.startsWith("# ' "))
            normalComment = comment.substring(0, comment.length() - 1).replaceFirst("# ' ", "# "); else {
  ...
  private void copyResource(InputStream resource, File file)
  {
    try
    {
      ...
      byte[] buf = new byte['Ѐ'];
      int length;
      while ((length = resource.read(buf)) > 0) { int length;
